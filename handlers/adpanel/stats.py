...
async def handle_stats(client: Client, message):
    """Display bot statistics including all group IDs."""
    ...
    try:
        ...
        total_users = db.users.count_documents({"name": {"$exists": True}})
        ...
        logger.debug(f"Fetched counts: users={total_users}, groups={total_groups}, active={active_groups}, full={full_groups}, approved={approved_groups}, watermark={watermark}")
        ...
        message_text = (
            "📊 Bot Statistics\n\n"
            f"👤 Total Users with Submissions: {total_users}\n"
